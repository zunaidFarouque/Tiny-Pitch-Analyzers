...
bool isAllowedAudioExtension(const juce::String &extLowerNoDot) {
  static const juce::String allowed[] = {"wav", "aif", "aiff", "flac", "mp3",
                                         "ogg", "m4a", "aac",  "wma",  "caf"};
...
void collectAudioFilesFromDir(const juce::File &dir,
...
/** Mono float window [endSample-fftSize, endSample); zero-pad before file
 * start. */
void readMonoFloatWindow(juce::AudioFormatReader &reader, int fftSize,
                         int64_t endSample, std::vector<float> &dst) {
  ...
  const int toRead =
      static_cast<int>(juce::jmin<int64_t>(len, total - fileReadStart));
  if (toRead <= 0)
    return;
  ...
  for (int i = 0; i < toRead; ++i) {
    ...
    dst[static_cast<std::size_t>(dstOffset + i)] =
...
MainComponent::MainComponent() {
  ...
  addAndMakeVisible(waterfallEnergyLowSlider_);
  addAndMakeVisible(waterfallEnergyHighSlider_);
  addAndMakeVisible(waterfallAlphaPowLowSlider_);
  addAndMakeVisible(waterfallAlphaPowHighSlider_);
  addAndMakeVisible(waterfallAlphaThresholdSlider_);
  addAndMakeVisible(waterfallEnergyLowLabel_);
  addAndMakeVisible(waterfallEnergyHighLabel_);
  addAndMakeVisible(waterfallAlphaPowLowLabel_);
  addAndMakeVisible(waterfallAlphaPowHighLabel_);
  addAndMakeVisible(waterfallAlphaThresholdLabel_);
  addAndMakeVisible(waterfallShapingFreqLogSlider_);
  addAndMakeVisible(waterfallShapingFreqLogLabel_);
  addAndMakeVisible(preEmphasisToggle_);
  addAndMakeVisible(spectralSmearingToggle_);
  ...
  instantPreviewToggle_.setTooltip(
      "Instantaneous calculation of example/audio file: Waterfall mode "
      "pre-computes only the visible time window (384 analysis columns); "
  ...
  windowKindCombo_.onChange = [this] {
    ...
    engine_.state().setWindowKind(id == 2 ? pitchlab::WindowKind::Gaussian
                                          : pitchlab::WindowKind::Hanning);
  ...
  positionSlider_.textFromValueFunction = [](double secs) {
  ...
  waterfallEnergyLowLabel_.setText("W-Energy Low", juce::dontSendNotification);
  waterfallEnergyHighLabel_.setText("W-Energy High",
                                    juce::dontSendNotification);
  waterfallAlphaPowLowLabel_.setText("W-AlphaPow Low",
                                     juce::dontSendNotification);
  waterfallAlphaPowHighLabel_.setText("W-AlphaPow High",
                                      juce::dontSendNotification);
  waterfallAlphaThresholdLabel_.setText("W-Thresh", juce::dontSendNotification);
  waterfallShapingFreqLogLabel_.setText("W-Freq map", juce::dontSendNotification);

  auto setupWaterfallSlider = [](juce::Slider &s) {
    s.setSliderStyle(juce::Slider::LinearHorizontal);
    s.setTextBoxStyle(juce::Slider::TextBoxRight, false, 70, 20);
  };
  setupWaterfallSlider(waterfallEnergyLowSlider_);
  setupWaterfallSlider(waterfallEnergyHighSlider_);
  setupWaterfallSlider(waterfallAlphaPowLowSlider_);
  setupWaterfallSlider(waterfallAlphaPowHighSlider_);
  setupWaterfallSlider(waterfallAlphaThresholdSlider_);
  setupWaterfallSlider(waterfallShapingFreqLogSlider_);

  waterfallShapingFreqLogSlider_.setRange(0.0, 1.0, 0.01);
  waterfallShapingFreqLogSlider_.setValue(1.0, juce::dontSendNotification);
  waterfallShapingFreqLogSlider_.textFromValueFunction = [](double v) {
    if (v <= 0.02)
      return juce::String("Lin Hz");
    if (v >= 0.98)
      return juce::String("Log Hz");
    return juce::String(juce::roundToInt(v * 100.0)) + juce::String("% log");
  };

  waterfallEnergyLowSlider_.setRange(0.0f, 2.0f, 0.001f);
  waterfallEnergyHighSlider_.setRange(0.0f, 2.0f, 0.001f);
  waterfallAlphaPowLowSlider_.setRange(0.1f, 120.0f, 0.05f);
  waterfallAlphaPowHighSlider_.setRange(0.1f, 60.0f, 0.05f);
  waterfallAlphaThresholdSlider_.setRange(0.0f, 0.05f, 0.0005f);

  waterfallEnergyLowSlider_.setValue(1.0, juce::dontSendNotification);
  waterfallEnergyHighSlider_.setValue(1.0, juce::dontSendNotification);
  waterfallAlphaPowLowSlider_.setValue(2.0, juce::dontSendNotification);
  waterfallAlphaPowHighSlider_.setValue(2.0, juce::dontSendNotification);
  waterfallAlphaThresholdSlider_.setValue(0.0050, juce::dontSendNotification);

  auto pushWaterfallHostsIdentity = [this] {
    openGlHost_.setWaterfallEnergyScale(1.0f);
    openGlHost_.setWaterfallAlphaPower(1.0f);
    cpuHost_.setWaterfallEnergyScale(1.0f);
    cpuHost_.setWaterfallAlphaPower(1.0f);
  };

  waterfallEnergyLowSlider_.onValueChange = [this, pushWaterfallHostsIdentity] {
    engine_.state().wEnergyLow.store(
        (float)waterfallEnergyLowSlider_.getValue(), std::memory_order_relaxed);
    pushWaterfallHostsIdentity();
    bumpInstantPreviewDebounced();
  };
  waterfallEnergyHighSlider_.onValueChange = [this,
                                              pushWaterfallHostsIdentity] {
    engine_.state().wEnergyHigh.store(
        (float)waterfallEnergyHighSlider_.getValue(),
        std::memory_order_relaxed);
    pushWaterfallHostsIdentity();
    bumpInstantPreviewDebounced();
  };
  waterfallAlphaPowLowSlider_.onValueChange = [this,
                                               pushWaterfallHostsIdentity] {
    engine_.state().wAlphaPowLow.store(
        (float)waterfallAlphaPowLowSlider_.getValue(),
        std::memory_order_relaxed);
    pushWaterfallHostsIdentity();
    bumpInstantPreviewDebounced();
  };
  waterfallAlphaPowHighSlider_.onValueChange = [this,
                                                pushWaterfallHostsIdentity] {
    engine_.state().wAlphaPowHigh.store(
        (float)waterfallAlphaPowHighSlider_.getValue(),
        std::memory_order_relaxed);
    pushWaterfallHostsIdentity();
    bumpInstantPreviewDebounced();
  };
  ...
  waterfallShapingFreqLogSlider_.onValueChange = [this] {
    engine_.state().wShapingFreqLogBlend.store(
        (float)waterfallShapingFreqLogSlider_.getValue(), std::memory_order_relaxed);
    bumpInstantPreviewDebounced();
  };

  preEmphasisToggle_.setToggleState(false, juce::dontSendNotification);
  engine_.state().enablePreEmphasis.store(false, std::memory_order_relaxed);
  preEmphasisToggle_.onClick = [this] {
    engine_.state().enablePreEmphasis.store(preEmphasisToggle_.getToggleState(),
                                            std::memory_order_relaxed);
    bumpInstantPreviewDebounced();
  };

  spectralSmearingToggle_.setToggleState(true, juce::dontSendNotification);
  engine_.state().spectralSmearingEnabled.store(true,
                                                std::memory_order_relaxed);
  spectralSmearingToggle_.onClick = [this] {
    engine_.state().spectralSmearingEnabled.store(
        spectralSmearingToggle_.getToggleState(), std::memory_order_relaxed);
    bumpInstantPreviewDebounced();
  };
  ...
  agcEnabledToggle_.onClick = [this] {
    engine_.state().agcEnabled.store(agcEnabledToggle_.getToggleState(),
                                     std::memory_order_relaxed);
    bumpInstantPreviewDebounced();
  };
  ...
  agcStrengthSlider_.onValueChange = [this] {
    engine_.state().agcStrength.store((float)agcStrengthSlider_.getValue(),
                                      std::memory_order_relaxed);
    bumpInstantPreviewDebounced();
  };
  ...
  foldInterpCombo_.onChange = [this] {
    ...
    if (id == 1)
      m = pitchlab::FoldInterpMode::Nearest;
    else if (id == 3)
      m = pitchlab::FoldInterpMode::Quadratic3Bin;
  ...
  foldWeightCombo_.onChange = [this] {
    ...
    pitchlab::FoldHarmonicWeightMode m =
        pitchlab::FoldHarmonicWeightMode::InvSqrtH;
    if (id == 1)
      m = pitchlab::FoldHarmonicWeightMode::Uniform;
    else if (id == 2)
      m = pitchlab::FoldHarmonicWeightMode::InvH;
  ...
  foldOctavesCombo_.onChange = [this] {
    ...
    engine_.state().foldMaxOctaves.store(id == 0x100 ? 0 : id,
                                         std::memory_order_relaxed);
    bumpInstantPreviewDebounced();
  };
  ...
  waterfallCurveCombo_.onChange = [this] {
    pitchlab::WaterfallDisplayCurveMode m =
        pitchlab::WaterfallDisplayCurveMode::Sqrt;
    if (waterfallCurveCombo_.getSelectedId() == 1)
      m = pitchlab::WaterfallDisplayCurveMode::Linear;
    else if (waterfallCurveCombo_.getSelectedId() == 3)
      m = pitchlab::WaterfallDisplayCurveMode::LogDb;
  ...
  chromaShapingCombo_.onChange = [this] {
    ...
    case 2:
      m = pitchlab::ChromaShapingMode::LogCompress;
      break;
    case 3:
      m = pitchlab::ChromaShapingMode::NoiseFloorSubtract;
      break;
    case 4:
      m = pitchlab::ChromaShapingMode::PercentileGate;
      break;
    default:
      break;
    }
  ...
  foldModelCombo_.onChange = [this] {
    engine_.state().setFoldHarmonicModel(
        foldModelCombo_.getSelectedId() == 2
  ...
  spectralBackendCombo_.onChange = [this] {
    ...
    if (spectralBackendCombo_.getSelectedId() == 2)
      m = pitchlab::SpectralBackendMode::ConstQApprox_v0_1;
    else if (spectralBackendCombo_.getSelectedId() == 3)
      m = pitchlab::SpectralBackendMode::VariableQApprox_v0_1;
    else if (spectralBackendCombo_.getSelectedId() == 4)
      m = pitchlab::SpectralBackendMode::MultiResSTFT_v1_0;
    ...
    if (sampleRate_ > 0.0 && samplesPerBlockExpected_ > 0) {
      ...
      const int maxBlock =
          juce::jmax(samplesPerBlockExpected_, engine_.state().fftSize);
  ...
  analysisRateCombo_.onChange = [this] {
    ...
    engine_.state().analysisEveryNCallbacks.store(every,
                                                  std::memory_order_relaxed);
    updateInstantPreviewChrome();
    bumpInstantPreviewDebounced();
  };
  ...
  highPassSlider_.textFromValueFunction = [](double v) {
    return v < static_cast<double>(pitchlab::EngineState::kHighPassOffHz)
               ? juce::String("Off")
               : juce::String(v, 0) + " Hz";
  };
  highPassSlider_.setValue(0.0, juce::dontSendNotification);
  highPassSlider_.onValueChange = [this] {
    engine_.state().highPassCutoffHz.store(
        static_cast<float>(highPassSlider_.getValue()),
        std::memory_order_relaxed);
    bumpInstantPreviewDebounced();
  };

  // Apply defaults once.
  waterfallEnergyLowSlider_.onValueChange();
  waterfallEnergyHighSlider_.onValueChange();
  waterfallAlphaPowLowSlider_.onValueChange();
  waterfallAlphaPowHighSlider_.onValueChange();
  waterfallAlphaThresholdSlider_.onValueChange();
  waterfallShapingFreqLogSlider_.onValueChange();
  ...
  std::unique_ptr<juce::XmlElement> savedAudioState =
      juce::parseXML(getAudioDeviceSettingsFile());
...
void MainComponent::prepareToPlay(int samplesPerBlockExpected,
  ...
  engine_.prepareToPlay(sampleRate, samplesPerBlockExpected);
  {
    const juce::ScopedLock sl(offlineMutex_);
    const int maxBlock =
        juce::jmax(samplesPerBlockExpected, engine_.state().fftSize);
...
void MainComponent::getNextAudioBlock(
...

  {
    ...
    if (inputMode_ == InputMode::FilePlayback && readerSource_ != nullptr) {
      // AudioSourcePlayer copies inputs into the output buffer first; clear so
      // file playback is not mixed with (possibly silent) mic pass-through.
    ...
    // LiveMicrophone: AudioSourcePlayer already copied device input into buffer
    // for pass-through.
  }
  ...
  for (int c = 0; c < ch; ++c)
    channelPtrScratch_[static_cast<std::size_t>(c)] =
        buf->getReadPointer(c, start);
  ...
  while (pk > prev && !audioPeakHold_.compare_exchange_weak(
                          prev, pk, std::memory_order_relaxed)) {
  }
  ...
  const bool suppressWaterfall =
      instantPreviewToggle_.getToggleState() &&
      inputMode_ == InputMode::FilePlayback && currentReader_ != nullptr &&
      mapComboIdToMode(vizModeCombo_.getSelectedId()) ==
          VisualizationMode::Waterfall;

  if (!suppressWaterfall)
    activeRenderer_->pushWaterfallRow(
        std::span<const float>{frame.chromaRow.data(), frame.chromaRow.size()});
}
...
void MainComponent::resized() {
  ...
  agcStrengthSlider_.setBounds(
      fftRow.removeFromLeft(fftRow.getWidth() / 2).reduced(0, 2));

  r.removeFromTop(4);
  auto hpRow = r.removeFromTop(24);
  highPassLabel_.setBounds(hpRow.removeFromLeft(56).reduced(0, 2));
  highPassSlider_.setBounds(
      hpRow.removeFromLeft(hpRow.getWidth() / 2).reduced(0, 2));
  ...
  auto ctrlRow1 = r.removeFromTop(56);
  const int segW1 = ctrlRow1.getWidth() / 3;
  {
    auto s0 = ctrlRow1.removeFromLeft(segW1);
    waterfallEnergyLowLabel_.setBounds(s0.removeFromTop(14));
    waterfallEnergyLowSlider_.setBounds(s0.reduced(0, 2));
    auto s1 = ctrlRow1.removeFromLeft(segW1);
    waterfallEnergyHighLabel_.setBounds(s1.removeFromTop(14));
    waterfallEnergyHighSlider_.setBounds(s1.reduced(0, 2));
    auto s2 = ctrlRow1;
    waterfallAlphaThresholdLabel_.setBounds(s2.removeFromTop(14));
    waterfallAlphaThresholdSlider_.setBounds(s2.reduced(0, 2));
  }

  r.removeFromTop(4);
  auto ctrlRow2 = r.removeFromTop(64);
  const int segW2 = ctrlRow2.getWidth() / 3;
  {
    auto t0 = ctrlRow2.removeFromLeft(segW2);
    waterfallAlphaPowLowLabel_.setBounds(t0.removeFromTop(14));
    waterfallAlphaPowLowSlider_.setBounds(t0.reduced(0, 2));
    auto t1 = ctrlRow2.removeFromLeft(segW2);
    waterfallAlphaPowHighLabel_.setBounds(t1.removeFromTop(14));
    waterfallAlphaPowHighSlider_.setBounds(t1.reduced(0, 2));
    auto t2 = ctrlRow2;
    auto t2a = t2.removeFromTop(t2.getHeight() / 2);
    preEmphasisToggle_.setBounds(t2a.reduced(0, 2));
    spectralSmearingToggle_.setBounds(t2.reduced(0, 2));
  }

  r.removeFromTop(4);
  auto ctrlRow3 = r.removeFromTop(52);
  waterfallShapingFreqLogLabel_.setBounds(ctrlRow3.removeFromTop(14));
  waterfallShapingFreqLogSlider_.setBounds(ctrlRow3.reduced(0, 2));
  ...
  if (showInstantScroll) {
    const int hop = samplesPerBlockExpected_ *
                    juce::jmax(1, engine_.state().analysisEveryNCallbacks.load(
                                      std::memory_order_relaxed));
    const double visibleSec =
        static_cast<double>(SharedWaterfallRing::kRows * hop) / sampleRate_;
...
void MainComponent::timerCallback() {
  ...
  engineLabel_.setText(
      "Engine " + juce::String(pitchlab::engineVersionString()) +
...
void MainComponent::updateDeviceAndPeakLabels() {
  if (auto *dev = deviceManager.getCurrentAudioDevice()) {
    audioDeviceLabel_.setText(
        dev->getName() + "  " + juce::String(dev->getCurrentSampleRate(), 0) +
            " Hz  " + juce::String(dev->getCurrentBufferSizeSamples()) + " smp",
        juce::dontSendNotification);
  } else {
...
void MainComponent::audioSettingsClicked() {
  auto *audioSettingsComp = new juce::AudioDeviceSelectorComponent(
      deviceManager, 0, 2, 0, 2, false, false, true, true);
...
void MainComponent::syncRendererBackend() {
  IRendererHost *next =
      shouldUseCpuBackend(renderBackendPolicy_, openGlHost_.isBackendHealthy())
...
void MainComponent::openFileClicked() {
  fileChooser_ = std::make_unique<juce::FileChooser>(
      "Open audio file", juce::File{}, kAudioFileChooserWildcards);
...
void MainComponent::loadAudioFile(const juce::File &file) {
  ...
  // stop() waits for the audio thread; never hold transportLock_ during stop()
  // or the audio callback cannot enter its critical section and we deadlock (no
  // playback).
  transport_.stop();
  ...
  readerSource_ = std::make_unique<juce::AudioFormatReaderSource>(
      currentReader_.get(), false);

  if (auto *dev = deviceManager.getCurrentAudioDevice()) {
    transport_.setSource(readerSource_.get(), 0, nullptr,
                         currentReader_->sampleRate);
    transport_.prepareToPlay(dev->getCurrentBufferSizeSamples(),
...
void MainComponent::refreshExampleAudioList() {
  ...
  std::sort(filtered.begin(), filtered.end(),
            [](const juce::File &a, const juce::File &b) {
              return a.getFullPathName().compareIgnoreCase(
                         b.getFullPathName()) < 0;
            });
  filtered.erase(std::unique(filtered.begin(), filtered.end(),
                             [](const juce::File &a, const juce::File &b) {
                               return a.getFullPathName().equalsIgnoreCase(
                                   b.getFullPathName());
                             }),
                 filtered.end());
  std::sort(filtered.begin(), filtered.end(),
  ...
  if (defaultWav.existsAsFile()) {
    for (int i = 0; i < static_cast<int>(exampleAudioFiles_.size()); ++i) {
      if (exampleAudioFiles_[static_cast<std::size_t>(i)]
              .getFullPathName()
              .equalsIgnoreCase(defaultWav.getFullPathName())) {
        selectId = i + 2;
        break;
      }
    }
...
void MainComponent::fftSizeChanged() {
  ...
  {
    const juce::ScopedLock sl(offlineMutex_);
    offlineEngine_.reconfigureFftSize(
        newFft, sampleRate_, juce::jmax(samplesPerBlockExpected_, newFft));
  }
...
void MainComponent::updatePositionFromTransport() {
  ...
  const juce::ScopedLock sl(transportLock_);
  if (transport_.isPlaying())
    positionSlider_.setValue(transport_.getCurrentPosition(),
                             juce::dontSendNotification);
}
...
void MainComponent::syncOfflineEngineFromLive(double analysisSampleRate) {
  ...
  O.agcEnabled.store(L.agcEnabled.load(std::memory_order_relaxed),
                     std::memory_order_relaxed);
  O.agcStrength.store(L.agcStrength.load(std::memory_order_relaxed),
                      std::memory_order_relaxed);
  O.agcNoiseFloor.store(L.agcNoiseFloor.load(std::memory_order_relaxed),
                        std::memory_order_relaxed);
  O.temporalRelease.store(L.temporalRelease.load(std::memory_order_relaxed),
                          std::memory_order_relaxed);
  O.setWindowKind(L.windowKind());
  O.setFoldInterpMode(L.foldInterpMode());
  O.setFoldHarmonicWeightMode(L.foldHarmonicWeightMode());
  O.foldMaxOctaves.store(L.foldMaxOctaves.load(std::memory_order_relaxed),
                         std::memory_order_relaxed);
  ...
  O.analysisEveryNCallbacks.store(
      L.analysisEveryNCallbacks.load(std::memory_order_relaxed),
      std::memory_order_relaxed);
  O.highPassCutoffHz.store(L.highPassCutoffHz.load(std::memory_order_relaxed),
                           std::memory_order_relaxed);
  O.maxHarmonicK.store(L.maxHarmonicK.load(std::memory_order_relaxed),
                       std::memory_order_relaxed);
  O.wEnergyLow.store(L.wEnergyLow.load(std::memory_order_relaxed),
                     std::memory_order_relaxed);
  O.wEnergyHigh.store(L.wEnergyHigh.load(std::memory_order_relaxed),
                      std::memory_order_relaxed);
  O.wAlphaPowLow.store(L.wAlphaPowLow.load(std::memory_order_relaxed),
                       std::memory_order_relaxed);
  O.wAlphaPowHigh.store(L.wAlphaPowHigh.load(std::memory_order_relaxed),
                        std::memory_order_relaxed);
  O.wShapingFreqLogBlend.store(
      L.wShapingFreqLogBlend.load(std::memory_order_relaxed),
      std::memory_order_relaxed);
  O.enablePreEmphasis.store(L.enablePreEmphasis.load(std::memory_order_relaxed),
                            std::memory_order_relaxed);
  O.spectralSmearingEnabled.store(
      L.spectralSmearingEnabled.load(std::memory_order_relaxed),
      std::memory_order_relaxed);
}
...
void MainComponent::updateInstantPreviewChrome() {
  const bool showBar = instantPreviewToggle_.getToggleState() &&
                       inputMode_ == InputMode::FilePlayback &&
  ...
  instantScrollBar_.setCurrentRange(start, visibleSec,
                                    juce::dontSendNotification);
}
...
void MainComponent::launchInstantWaterfallJob() {
  if (!instantPreviewToggle_.getToggleState() ||
      ...
      mapComboIdToMode(vizModeCombo_.getSelectedId()) !=
          VisualizationMode::Waterfall ||
      !currentAudioFileForInstant_.existsAsFile() || sampleRate_ <= 0.0 ||
      samplesPerBlockExpected_ <= 0)
    return;

  ++instantJobGeneration_;
  const std::uint64_t gen =
      instantJobGeneration_.load(std::memory_order_relaxed);
  const juce::File file = currentAudioFileForInstant_;
  const double scrollStartSec =
      instantScrollStartSec_.load(std::memory_order_relaxed);
  const int hopDevice =
      samplesPerBlockExpected_ *
      juce::jmax(1, engine_.state().analysisEveryNCallbacks.load(
                        std::memory_order_relaxed));
  const double deviceSr = sampleRate_;
  const int fftSnap = engine_.state().fftSize;

  juce::Thread::launch([this, gen, file, scrollStartSec, hopDevice, deviceSr,
                        fftSnap] {
    ...
    const int hopFile =
        juce::jmax(1, (int)std::llround((double)hopDevice * srFile / deviceSr));

    {
      ...
      const int offlineNativeSamples =
          offlineEngine_.offlineMonoInputSampleCount();
      offlineEngine_.reconfigureFftSize(
          fftSnap, srFile,
          juce::jmax(hopFile, juce::jmax(fftSnap, offlineNativeSamples)));
      offlineEngine_.state().audioBufferSize = hopFile;

      const std::int64_t len = reader->lengthInSamples;
      const std::int64_t w0 =
          (std::int64_t)std::llround(scrollStartSec * srFile);
      ...
      for (int col = 0; col < kCols; ++col) {
        ...
        const std::int64_t endSample =
            w0 + static_cast<std::int64_t>(col + 1) *
                     static_cast<std::int64_t>(hopFile);
        if (endSample <= 0 || len <= 0)
          continue;

        readMonoFloatWindow(*reader, offlineNativeSamples, endSample, win);
        offlineEngine_.analyzeOfflineWindowFromMonoFloat(
            std::span<const float>{win.data(), win.size()}, frame);
        std::copy(frame.chromaRow.begin(), frame.chromaRow.end(),
                  grid.begin() + static_cast<std::size_t>(col) *
    ...
    juce::MessageManager::callAsync(
        [this, gen, grid = std::move(grid)]() mutable {
          if (gen != instantJobGeneration_.load(std::memory_order_relaxed))
            return;
          activeRenderer_->commitWaterfallGrid384(
              std::span<const float>{grid.data(), grid.size()});
        });
  });
}
